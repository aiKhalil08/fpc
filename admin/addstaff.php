...
if (@$_REQUEST['id']) {
	$id = $_GET['id'];
    if (Database::check_unique_id($id, 'staffs')) {
        header('location: ./index.php');
        die();
    }
	$existing = true;
	$obj = new Get_Info('staff',$id);//declared in contr.php
	$array = $obj->get_infof();
}
...
<body>
	...
	<main>
		...
		<section class='forms  add_forms'>
            ...
            <article>   
                <p>Fill the following form appropriately to register a new staff.</p>
                <p>* Any mistake that occurs during the registration can only be rectified after the registration is complete.</p>
                <form method='post' action='./edit_bio.php?pe=fa' class='reg_form'>
                    <fieldset>
...
<script>
    ...
    Array.from(form.elements).forEach(input => {
        if (input.type != 'hidden' && input.type != 'submit' && input.type != 'fieldset') {
            let type = 'general';
            if (input.name == 'staff_first_name' || input.name == 'staff_last_name') type = 'only_string';
