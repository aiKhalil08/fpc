...
<body>
	...
	<main>
        <?=check_js_enabled();?>
		<section class='forms delete_forms'>
            <h3>DELETE STAFF</h3>
            <?=handle_session_errors();?>
			<article>
                <p>Input the username(s) of staff(s) you want to delete:</p> 
                <p>* Separate usernames by comma(,) when inputing more than one username.</p> 
                <form method='post' action='./delete_st.php?type=fa' class='delete_form'>
                    <p><input type='text' name='usernames' placeholder='enter username(s)'>
                    <input type="submit" name="delete" id="sel_delete_par" value='Delete'></p>
                </form>
			</article>
        </section>
    </main>
