...
if (isset($_POST['search'])) {
	...
	$search = new Search_St('staff', $method, $input);//declared in contr.php
	$result = $search->search();
	$view = new Display_All_St($result, 'staff');//declared in view.php
}
?>
<!doctype html>
<html>
...
<body>
	...
	<main>
		...
		<section class='forms search_forms'>
			...
			<article class='input'>
			<form method='post' action='./search_staff.php' class='search_form'>
				<?php 
				if (!isset($_POST['search'])) {
				?>
					<p>Choose parameter to search by:</p>
					<p>
						<label>Search staff by: </label>
					...
					$methods = array('role' => 'or', 'surname' => 'us', 'username' => 'su');
				?>
					<p>Choose parameter to search by:</p>
					<p>
						<label>Search staff by: </label>
				...
				<script>
				createInputField(document.querySelector('form.search_form'), '<?=$_POST["search_par"]?>', '<?=$_POST["input"]?>', 'staff');
				</script>
				<?php } ?>
...
<script>
	document.querySelector('form.search_form #search_par').addEventListener('change', function (e) {
		...
		createInputField(this.form, selected.value, false, 'staff');
	});
</script>
</html>