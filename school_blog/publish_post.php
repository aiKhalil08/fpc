...
check_request(['article_title', 'article_body'], './index.php');


$title = $_POST['article_title'];
$image = !empty($_FILES['article_image']['name']) ? $_FILES['article_image'] : null;
$body = $_POST['article_body'];


$publish = publish_post($title, $image, $body); //function validates input and returns publish object. declared in contr.php
...
<body>
	...
	<main>
        <?=check_js_enabled();?>
        <section class='publish_blog'>
            <?php if ($id = $publish->create_post()) {
                $post = $publish->retrieve_post($id);
            }?>
            <div><strong>THE POST HAS BEEN SUCCESSFULLY PUBLISHED.</strong></div>
