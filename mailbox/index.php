...
if (isset($_POST['search_submit'])) {
    $received_mails = new View_Mails($type, $username, 'received', trim($_POST['search_text']));//declared in contr.php
} else {
    $received_mails = new View_Mails($type, $username, 'received');//declared in contr.php
}
$new_mails_count = $received_mails->new_mails();
$mails = $received_mails->amalg_view_mails();
?>
<!doctype html>
<html>
<head>
...
<body>
	...
	<main>
        ...
        <section class='mails'>
            ...
            <?php if (!isset($_GET['e'])) {?>
            <article class='received_mails mails_list'>
                <header>
                    <span>Received mails</span>

                    <span>new: <span id='new_count'><?=$new_mails_count?></span></span>

                    <span>all: <span id='all_count'><?=!$mails->count_all_mails() ? '0' : $mails->count_all_mails();?></span></span>

                    <form method='post' action='<?='./index.php?pe='.base64_encode($type).'&me='.base64_encode($username)?>' class='search_form'><input type='search' name='search_text' placeholder='search mails by title...' value='<?=isset($_POST['search_submit']) ? $_POST['search_text']: '';?>'><button name='search_submit'><i class="fa-solid fa-magnifying-glass"></i></button></form>

                </header>
                <?php
                try {
                    echo $mails->ret_mails($new_mails_count);
                } catch (CustomException $e) {
                    handle_exception_obj($e);
                    Error_Reg::set_errors('Something went wrong, please try again.');
                    Error_Reg::set_err_session();
                    header('location: ./index.php?pe='.base64_encode($type).'&me='.base64_encode($username).'&e=1');
                    die();
                }
                ?>
            </article>
