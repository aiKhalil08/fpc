...
if (isset($_POST['search_submit'])) {
    $sent_mails = new View_Mails($type, $username, 'sent', trim($_POST['search_text']));//declared in contr.php
} else {
    $sent_mails = new View_Mails($type, $username, 'sent');//declared in contr.php
}

$mails = $sent_mails->amalg_view_mails();
?>
<!doctype html>
<html>
<head>
	...
	<main>
        ...
        <section class='mails'>
            ...
            <article class='received_mails mails_list'>
                <header><span>Sent mails</span><form method='post' action='<?='./sent_mails.php?pe='.base64_encode($type).'&me='.base64_encode($username)?>' class='search_form'><input type='search' name='search_text' placeholder='search mails by title...' value='<?=isset($_POST['search_submit']) ? $_POST['search_text']: '';?>'><button name='search_submit'><i class="fa-solid fa-magnifying-glass"></i></button></form><span>all: <span id='all_count'><?=!$mails->count_all_mails() ? '0' : $mails->count_all_mails();?></span></span></header>
                <?php
                try {
                   echo $mails->ret_mails();
